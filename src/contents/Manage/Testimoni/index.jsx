...
function Testimoni() {
  ...
  const [tableHead, tableHeadSet] = useState([
    ...
    { Header: "Rating", accessor: "rating", width: "25%" },
  ...
  const loadData = (params) => {
    ...
    useAxios()
      .post(`${Config.ApiUrl}/api/v1/manage/testimoni/list`, payload)
      .then((response) => {
        ...
        const output = data.data.map((item) => {
          ...
          return {
            ...
            rating: item.rating,
            testimoni: <p style={{ wordWrap: "break-word", width: "25em" }}>{item.testimonial}</p>,
            action:
