...
function AccountLevel() {
  ...
  const [tableHead, tableHeadSet] = useState([
    ...
    { Header: "Nama Level", accessor: "name", width: "25%" },
  ...
  const loadData = (params) => {
    ...
    useAxios()
      .post(`${Config.ApiUrl}/api/v1/master/account-level/list`, payload)
      .then((response) => {
      ...
      <MDBox pb={3} my={3}>
        <MDBox pb={2} mt={{ xs: 2, md: 0 }} display="flex">
          <MDButton
            ...
            variant="gradient"
            component={Link}
            to={{ pathname: "/master/account-level/add" }}
          >
            Tambah
          </MDButton>
        </MDBox>
